...
public class TopScoreService : MonoBehaviour {
	...
	private string _addressOwner = "0x7b785ecaa853b3f6f26fd4aed5ad968cfe17bfda";
	private string _userAddress;
	private byte[] _key;
	private string _privateKey = "<Insert private key here...I have removed since upping to git>";
		...
		while (true) {
			...
			if (_userAddress != null) {
				...
				if (userTopScoreRequest.Exception == null) {
					...
					GameManager.Instance.TopScoreRecorded = topScoreUser;
					wait = 3;
				} else {
	...
	public IEnumerator CheckSubmitScore () {
		...
		while (true) {
			...
			if (GameManager.Instance.SubmitTopScore && !submitting) {
				if (_userAddress != null) {
					submitting = true;
					...
					var transactionInput = _scoreContractService.CreateSetTopScoreTransactionInput (_userAddress, _addressOwner, _privateKey,
						...
						//IMPORTANT!!  This through me off for a while....this must match the max gas limit used when creating the contract on remix (or atleast what I seemed to figure out)
					...
					if (ExternalProvider) {
						...
						var transactionSignedRequest = new TransactionSignedUnityRequest (_url, GameManager.instance.Key, _userAddress);
				...
				GameManager.Instance.SubmitTopScore = false;
				submitting = false;
			}
		}
	}
